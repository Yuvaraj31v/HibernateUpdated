...
public class EmployeeController {
    ...
    */
	...
	while(flag) {
	    ...
	    try {
	        dateOfBirth = LocalDate.parse(employeeDateOfBirth);
		flag = false;
		break;
            }
	...
	while(flag) {
	    ...
	    String[] dateOfJoin = scanner.next().split("-");
	    if (dateOfJoin[0].length() == 2) {
	        try {
		    JoiningDate=dateOfJoin[2]+"-"+dateOfJoin[1]+"-"+dateOfJoin[0];
		    employeeDateOfJoin = LocalDate.parse(JoiningDate);
		    if(Validation.getExperience(employeeDateOfJoin) < 0 ) { ;
			flag = true;
		    }
		    else {
			flag = false;
	    ...
	    else if (dateOfJoin[0].length()== 4) {
	        try {
		    JoiningDate=dateOfJoin[0]+"-"+dateOfJoin[1]+"-"+dateOfJoin[2];
		    employeeDateOfJoin = LocalDate.parse(JoiningDate);
		    if(Validation.getExperience(employeeDateOfJoin) < 0 ) { ;
			flag = true;
		    }
